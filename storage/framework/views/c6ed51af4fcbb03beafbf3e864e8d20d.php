<?php $__env->startSection('title', 'Rates Management'); ?>
    ...
    <table class="rates-table">
        ...
        <tbody id="generalRatesTable">
            <?php $__empty_1 = true; $__currentLoopData = $rates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr>
                    <td><?php echo e($rate->rate_name ?? $rate->duration_type . ' Rate'); ?></td>
                    <td>Per <?php echo e($rate->duration_type === 'Daily' ? 'Day' : ($rate->duration_type === 'Weekly' ? 'Week' : 'Month')); ?></td>
                    <td class="price-display">₱<?php echo e(number_format($rate->base_price, 2)); ?></td>
                    <td>
                        <?php if($rate->utilities && $rate->utilities->count() > 0): ?>
                            <ul style="margin: 0; padding-left: 1.25rem; list-style-type: disc;">
                                <?php $__currentLoopData = $rate->utilities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $utility): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li><?php echo e($utility->name); ?> - ₱<?php echo e(number_format($utility->price, 2)); ?></li>
                    ...
                    <td>
                        <div class="action-buttons">
                            <?php if(auth()->check() && strtolower(auth()->user()->role) === 'owner'): ?>
                            <button class="btn-edit" onclick="editRate(<?php echo e($rate->rate_id); ?>)">
                                <i class="bi bi-pencil-square"></i> Edit
                            </button>
                            <?php if($rate->status === 'active'): ?>
                                <button class="btn-archive" onclick="archiveRate(<?php echo e($rate->rate_id); ?>)">
                                    <i class="bi bi-archive"></i> Archive
                                </button>
                            <?php else: ?>
                                <button class="btn-activate" onclick="archiveRate(<?php echo e($rate->rate_id); ?>)">
...
<div class="modal fade" id="createRateModal" tabindex="-1" aria-labelledby="createRateModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?php echo e(route('rates.store')); ?>" method="POST" id="createRateForm">
                <?php echo csrf_field(); ?>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="rate_name" class="form-label">Rate Name <span class="text-muted">(Optional)</span></label>
                        <input type="text" class="form-control <?php $__errorArgs = ['rate_name'];
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control <?php $__errorArgs = ['base_price'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> is-invalid <?php unset($message);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\hmmth\sanasa_dormitory\resources\views/contents/rates.blade.php ENDPATH**/ ?>